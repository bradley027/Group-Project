...
# see http://github.com/hetland/octant
import octant
import octant.roms
...
class mch_animation(object):
    ...
    def new_frame(self, n):
        ...
        self.basemap.drawmeridians(range(-97, -85, 1), labels=[0, 0, 0, 0],
                                   color='0.5', linewidth=0.25, dashes=[1, 0])
...
if __name__ == '__main__':
    ...
    mch = mch_animation(YEAR,
                          '/home/rob/Projects/mch/archive/mch_his_*.nc', 
                          'timescale_frames',
                          grdfile='/home/rob/Projects/mch/mch_grd.nc')
    
    dates = np.load('/home/rob/Projects/mch/mch_dates.npy')
    years = np.array([d.year for d in dates])
    year_idx = np.where(years == YEAR)[0]
    
    ncr = octant.io.Dataset('/home/rob/Projects/mch/mch_river_frc.nc')
    river_time = octant.cf.time(ncr, name='river_time')
    Q = np.abs(ncr.variables['river_transport'][:]).sum(axis=1)*2.0/3.0
    river_years = np.array([d.year for d in river_time.dates])
    river_idx =np.where(river_years == YEAR)[0]
    
    ncw = octant.io.Dataset('/home/rob/Projects/mch/mch_wind_frc.nc')
    ...
    for tidx in range(tidx_start, tidx_end)[::12]:
        mch.new_frame(tidx)
        ...
        mch.filled_timeseries(river_time.dates[river_idx], Q[river_idx])
